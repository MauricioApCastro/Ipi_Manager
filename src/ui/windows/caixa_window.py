...
class CaixaWindow(QWidget):
    CATEGORIAS = ["Matrícula", "Mensalidade", "Crochê", "Tarot", "Filhos", "Outros"]
    ...
    def setup_ui(self):
        ...
        subtitulo = QLabel("Controle as entradas previstas e recebidas do mês.")
    ...
    def _criar_painel_lancamento(self):
        painel = self._painel_base("Entrada")
        ...
        self.txt_descricao = self._line_edit("Descrição")
        ...
        self.txt_observacoes = self._line_edit("Observações")
    ...
    def _criar_painel_mes(self):
        painel = self._painel_base("Entradas do mês")
        layout = painel.layout()

        filtros = QHBoxLayout()
        self.spin_mes = QSpinBox()
        self.spin_mes.setRange(1, 12)
        self.spin_mes.setPrefix("Mês ")
        ...
        cards = QHBoxLayout()
        self.card_previsto = self._card_total("Previsto", "R$ 0,00")
        self.card_recebido = self._card_total("Recebido", "R$ 0,00")
        self.card_pendente = self._card_total("Pendente", "R$ 0,00")
        cards.addWidget(self.card_previsto)
        cards.addWidget(self.card_recebido)
        cards.addWidget(self.card_pendente)

        self.tbl_entradas = QTableWidget(0, 6)
        self.tbl_entradas.setHorizontalHeaderLabels(["Data", "Descrição", "Categoria", "Valor", "Status", "Obs."])
    ...
    def carregar_dados(self):
        ...
        for row, entrada in enumerate(entradas):
            entrada_id, data, descricao, categoria, valor, status, observacoes = entrada
            ...
            self.tbl_entradas.setItem(row, 5, QTableWidgetItem(observacoes or ""))
    ...
    def excluir_entrada(self):
        if not self.entrada_selecionada_id:
            QMessageBox.warning(self, "Caixa", "Selecione uma entrada para excluir.")
            return
    ...
    def carregar_entrada_selecionada(self):
        ...
        self.combo_categoria.setCurrentText(entrada[3] or self.CATEGORIAS[0])
    ...
    def _dados_form(self):
        data = self.data_entrada.date().toString("yyyy-MM-dd")

        descricao = self.txt_descricao.text().strip()
        if not descricao:
            QMessageBox.warning(self, "Caixa", "Informe uma descrição.")
            return None
    ...
    def _atualizar_totais(self, entradas):
        ...
        self.card_previsto.findChild(QLabel, "valor").setText(self._formatar_moeda(previsto))
        self.card_recebido.findChild(QLabel, "valor").setText(self._formatar_moeda(recebido))
        self.card_pendente.findChild(QLabel, "valor").setText(self._formatar_moeda(pendente))
    ...
    def _texto_status(self, status):
        return "Recebido" if status == "RECEBIDO" else "Previsto"
